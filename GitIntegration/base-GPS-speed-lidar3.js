...
//var DataLog = fs.createWriteStream("Data.txt"); //for data logging info and writing to text file
//DataLog.once('open', function(fd) {//////////////////////////////////
//	DataLog.write(',\r\n');
//});
...
function LidarCheckAndTarget(string){ //
  ...
  if (lideg>=0 && lideg<=45){
	...
	} else if (600 <LidarDistance && LidarDistance<emergencyDist){
		LidarTarget = 0; //
...
io.on('connection', function SocketReadSensor(socket) {
	...
	SensorArd.analogRead(SpeedPin, function (CountSpeed) { //report cart speed
		 DistanceTraveled=((CountSpeed)*Math.PI*0.3)/4;//0.3 wheel diameter 4 number of magnets on the wheel
		 //console.log('count ' + CountSpeed)
...
io.on('connection', function SocketGPS(socket) {
	...
	serialPort.on('data', function(data){
		...
		if (data.charAt(data.length-5) == '*'){
		  ...
		  //console.log(londxn);
		  //console.log(latdxn);
		  GPStime1 =GPStime.slice(0,2);
		  GPStime2 = GPStime.slice(2,4);
		  GPStime3 = GPStime.slice(4,GPStime.length);
		  GPStime = GPStime1 + ":" + GPStime2 + ":" + GPStime3;
		  //console.log("Time of retrieval: " + GPStime + ". This is in UTC.");
		  //socket.emit('serialPort', latitude)
		}	
